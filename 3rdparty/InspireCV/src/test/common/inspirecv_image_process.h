...
public:
    ...
    inspirecv::Image ExecuteImageScaleProcessing(const float scale, bool with_rotation) {
        ...
        if (rotation_mode_ == ROTATION_90 || rotation_mode_ == ROTATION_270) {
            scaled_height = static_cast<int>(width_ * scale);
            scaled_width = static_cast<int>(height_ * scale);
        } else {
            scaled_height = static_cast<int>(height_ * scale);
            scaled_width = static_cast<int>(width_ * scale);
        }

        auto img_out = inspirecv::Image::Create(scaled_width, scaled_height, 3);
        std::shared_ptr<MNN::Tensor> tensor(MNN::Tensor::create<uint8_t>(
