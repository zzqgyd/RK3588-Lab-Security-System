...
int post_process_with_attrs(
    ...
    void *outputs,  // 实际传入的是 rknn_tensor_mem**
    letterbox_t *letter_box,
    float conf_threshold,
    float nms_threshold,
    object_detect_result_list *od_results)
{
    // ===== 关键修改：根据你的零拷贝方式选择正确的类型转换 =====
    // 因为你使用了 rknn_set_io_mem，所以 outputs 实际是 rknn_tensor_mem**
    ...
    {
        // ===== 使用 RV1106 的维度索引方式 =====
        int grid_h = worker_ctx->output_attrs[i].dims[1];  // RV1106 风格
        int grid_w = worker_ctx->output_attrs[i].dims[2];  // RV1106 风格
        int stride = model_in_h / grid_h;

        if (worker_ctx->is_quant)
        {
            // ===== 关键：使用 virt_addr 而不是 buf =====
            validCount += process_i8_rv1106(
                (int8_t *)output_mems[i]->virt_addr,  // ← 注意这里！
                (int *)anchor[i],
                grid_h, grid_w,
        ...
        else
        {
            validCount += process_fp32(
                (float *)output_mems[i]->virt_addr,  // ← 注意这里！
                (int *)anchor[i],
                grid_h, grid_w,
                model_in_h, model_in_w,
                stride,
